...
function onSelect(e: any) {
    ...
    const project = posts.value.find((project: IPost) => Number(project.id) === Number(e.target.id));
    const is_selected = selections.value.find((project: IPost) => Number(project.id) === Number(e.target.id));
    let selectionsClone = JSON.parse(JSON.stringify(selections.value));

    if (is_selected) {
        const index = selectionsClone.findIndex((project: IPost) => Number(project.id) === Number(e.target.id));
